...
describe("<Interview>", () => {
  const props = {
    ...
    interviews: [
      {
        writerPhoto: avatars["profileMemberA"],
      ...
      {
        writerPhoto: avatars["profileMemberB"],
        nutshell:
  ...
  it("props로 전달된 subtitle이 보여진다.", async () => {
    const { getByText } = renderInterview();

    getByText(props.subtitle);
  });
  it("props로 전달된 title 보여진다.", async () => {
    const { getByText } = renderInterview();
