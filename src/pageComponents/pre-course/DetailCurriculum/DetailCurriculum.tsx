...
const DetailCurriculum: React.FC = () => {
  const curriculumInfo: CodeTogetherCurriculumType[] = strainAllMdxInfo(
    useStaticQuery(PreCourseCurriculumQuery)
  );
  ...
  return (
    <DetailCurriculumWrapper>
      <TitleSetWrapper>
        <TitleSet subtitle={SUBTITLE.PRE_COURSE} title={TITLE.PRE_COURSE_DETAIL_CURRICULUM} />
      </TitleSetWrapper>
...
const PreCourseCurriculumQuery = graphql`
  query PreCourseCurriculumQuery {
    allMdx(
      sort: { order: ASC, fields: [frontmatter___index] }
      filter: { frontmatter: { templateKey: { glob: "preCourse_part*" } } }
    ) {
      edges {
        node {
          frontmatter {
