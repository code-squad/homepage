### 파일 디렉토리

`/content/main/master.mdx`

### 예시

```
---
templateKey: main_masters
masters:
  - image: crong
    field: 웹 프론트엔드
    name: 크롱
    introduce: Crong, 웹 프론트엔드 마스터
    nutshell: "“우리 모두 꾸준히 학습하며 성장하는 행복한 개발자가 되면 좋겠습니다.”"
    careers:
      - 우아한형제들 테크캠프 FE 교육 담당
      - 네이버 커넥트재단 부스트코스, 부스트캠프 FE교육 담당
      - 전 SK 플래닛 웹 FE 마스터
      - 전 NAVER FE 플랫폼팀 팀장
      - 전 NHN NEXT 웹 FE 전임 교수
      - 전 Initech, TmaxSoft 웹 보안 엔지니어
    schedules:
      - title: 마스터즈 코스
        subtitle: 웹 프론트엔드 클래스
        image: smallMastersCourse
        path: /masters

      - title: 코드투게더
        subtitle: 자바스크립트 Part. 2
        image: smallCodeTogether
        path: /code-together/javascript

  - image: honux
    field: 웹 백엔드
    name: 호눅스
    introduce: Honux, 웹 백엔드 마스터
    nutshell: "“남들하고 비교하면 초라해 보이지만 느리더라도 내 속도로 꾸준히 가면 잘 되겠죠.”"
    careers:
      - 현 코드스쿼드 웹 BE 교육 담당
      - 네카라쿠배 모두를 포함한 다수 기업을 대상으로 백엔드 및 클라우드 강의 경력
      - 전 Amazon Web Services 시니어 테크니컬 트레이너
      - 전 네이버 랩스 공동 연구원
      - 전 NHN NEXT DB 교수
      - 전 LG 전자 CTO 소프트웨어 플랫폼 연구소 선임 연구원
      - 한양대학교 전자컴퓨터공학 박사 졸업
    schedules:
      - title: 마스터즈 코스
        subtitle: 웹 백엔드 클래스
        image: smallMastersCourse
        path: /masters

  - image: ivy
    field: 모바일 Android
    name: 아이비
    introduce: Ivy, 모바일 Android 마스터
    nutshell: "“가치 있는 무언가를 만드는 과정을 즐길 수 있다면, 직접 부딪혀서 확인해보세요!”"
    careers:
      - 현 코드스쿼드, Udemy Android 앱 개발 강의
      - 2021 네이버 커넥트재단 부스트캠프 Android 앱 개발 강의
      - 전 ProtoPie Android 개발자, Product Manager
      - 전 KakaoPage Android 개발자
      - 전 NBT Android 개발자
    schedules:
      - title: 마스터즈 코스
        subtitle: 모바일 Android 클래스
        image: smallMastersCourse
        path: /masters

  - image: jk
    field: 모바일 iOS
    name: JK
    introduce: JK, 모바일 iOS 마스터
    nutshell: "“개발자가 된다는 것은 과거형이 아니라 현재 진행형이어야만 한다.”"
    careers:
      - 현 국내 최장수 macOS/iOS 개발자 커뮤니티와 레츠스위프트 운영진
      - 삼성전자, 네이버, 금결원, GE, 빙글 등 모바일 컨설팅/iOS 강의 경력
      - 전 레진코믹스 모바일 개발 리드
      - 전 NHN NEXT 모바일 전임 교수
      - 전 오로라 플래닛 모바일 스타트업 대표
      - 브리지텍 기술연구소 차장
    schedules:
      - title: 마스터즈 코스
        subtitle: 모바일 iOS 클래스
        image: smallMastersCourse
        path: /masters
---


```

### 항목별 설명

- masters: 마스터들을 보여줍니다.
  - image: 마스터의 이미지 파일 이름을 입력해야 합니다.
  - field: 탭 네비게이션에 보여지는 문구를 입력할 수 있습니다.
  - name: 마스터에 대한 설명중 가장 큰 글씨를 입력할 수 있습니다.
  - introduce: 마스터의 이름 옆에 있는 문구를 입력할 수 있습니다.
  - nutshell: 이름 바로 밑의 문구를 입력할 수 있습니다. 따옴표를 사용하고 싶다면 "“이와같이”" 입력하면 됩니다.
  - careers: 마스터의 경력 리스트를 입력할 수 있습니다. 의 형식으로 작성합니다.
  - schedules: 마스터가 곧 시작하게 되는 과정들에 대한 정보를 입력할 수 있습니다.
    - title: 코스에 대한 제목을 입력할 수 있습니다.
    - subtitle: 코스의 부제목을 입력할 수 있습니다.
    - image: 과정에 대한 이미지를 입력할 수 있습니다.
    - path: 해당 과정을 클릭했을 때 이동될 주소를 입력할 수 있습니다.
